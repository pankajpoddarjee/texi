...
<div class="toolbar" id="kt_toolbar">
	...
	<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
		...
		<div class="d-flex align-items-center py-1">
			<div class="">
				<a href="<?=base_url('Users/saveEmployee')?>" class="btn btn-primary" >
				...
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table" width="100%">
					...
					if(!empty($datas)){
						foreach($datas as $k=> $rows)
						{
							...
							$edit_link = base_url('Users/saveEmployee/'.$id);
					...
					?>
						<tr id="tr_<?=$rows->user_id?>">							
							...
							<td><?=$rows->address?></td>
							<td><?=$rows->staff_code?></td>
                                                        <td><?= ucfirst($rows->position_work)?></td>
							<td><?=$rows->email?><br><?=$rows->phone_code.'-'.$rows->phone_no?></td>
							<td data-filter="">
								<a href="<?=$status_link?>" class="badge badge-light-<?=($rows->status=='1')?'success':'danger'?>"><?=($rows->status=='1')?'Active':'Suspended'?></a>
							</td>
							
							<td class="text-end">								
								<button type="button" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edit<?=$rows->user_id?>">
