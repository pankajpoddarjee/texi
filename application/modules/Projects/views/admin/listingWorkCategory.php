...
<div class="modal fade " id="kt_modal_add" tabindex="-1" aria-hidden="true">
	...
	<div class="modal-dialog modal-dialog-centered mw-800px">
		...
		<div class="modal-content">
			<!--begin::Form-->
			<?php			
				$this->load->view('admin/save_work_type'); 
			?>
			<!--end::Form-->											
		</div>
	</div>
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table">
                ...
                if(!empty($datas)){
                        foreach($datas as $k=> $rows)
                        {
                                ...
                                $delete_link = base_url('Projects/projectWorkCategoryremove/'.$id);
                                $status_link = base_url('Projects/statusWorkCategoryChange/'.$id);
                                $query = $this->Project_model->getworkCategoryDetails($rows->id);
                                $edit['query']=$query;
                ...
                <tr id="tr_<?=$rows->id?>">							
                        ...
                        <td>
                                <a href="javascript:void()" class="text-gray-600 text-hover-primary mb-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edit<?=$rows->id?>"><?=$rows->category_name?></a>
                        </td>
                ...
                <div class="modal fade " id="kt_modal_edit<?=$rows->id?>" tabindex="-1" aria-hidden="true">
                        ...
                        <div class="modal-dialog modal-dialog-centered mw-800px">
                                ...
                                <div class="modal-content">
                                        <!--begin::Form-->
                                        <?php $this->load->view('admin/save_work_type', $edit); ?>
                                        <!--end::Form-->											
                                </div>
                        </div>
                </div>
...
$this->load->view('admin/_workjs', $this->data);
?>
