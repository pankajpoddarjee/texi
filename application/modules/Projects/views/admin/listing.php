...
<div class="toolbar" id="kt_toolbar">
    ...
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        ...
        <div class="d-flex align-items-center py-1">
            <div class="">
                <a href="<?= base_url('Projects/save') ?>" class="btn btn-primary" >
                    ...
                    <tbody class="fw-bold text-gray-600">
                        ...
                        if (!empty($datas)) {
                            foreach ($datas as $k => $rows) {
                                ...
                                $edit_link = base_url('Projects/save/' . $id);
                                $delete_link = base_url('Projects/remove/' . $id);
                                $status_link = base_url('Projects/statusChange/' . $id);
                                //$query = $this->User_model->getUserData($rows->user_id);
                                //$edit['query']=$query;
                                $building_type = str_replace('_', ' ', $rows->type);
                                ?>
                                <tr id="tr_<?= $rows->id ?>">							
                                    <td>
                                    ...
                                    <td>
                                        <a href="<?=$edit_link?>" class="text-gray-600 text-hover-primary mb-1"><?= $rows->project_name ?></a>
                                    </td>
                                    <td><?= ucfirst($building_type) ?></td>

                                    <td data-filter="">
                                        <a href="<?= $status_link ?>" class="badge badge-light-<?= ($rows->status == '0') ? 'success' : 'danger' ?>"><?= ($rows->status == '1') ? 'Running' : 'Completed' ?></a>
                                    </td>
                                    <td class="text-end">								
                                        <a href="<?=$edit_link?>">
